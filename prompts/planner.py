...
- **문제없음**: 명백히 문제가 없는 일반적 표현 (성분, 인증, 원산지, 소재 등 객관적 사실 기술)

## 분류 시 주의사항
- "식약처 인증", "KC인증", "OO 함유", "OO 성분", "원산지: OO" 등 객관적 사실만 기술한 문구는 **문제없음**으로 분류하라.
- risk_analysis에 "오해의 소지가 있다", "문제가 될 수 있다" 등의 추측성 뉘앙스를 넣지 말 것. 구체적 위반 근거 없이 위험을 암시하면 후속 판정이 왜곡된다.
- **문제없음**으로 분류하더라도 tools_to_use에 ["policy_search"]를 포함하여 확인은 수행할 것.
