...
def _strip_case_header(text: str) -> str:
...
class CaseAgent:
    ...
    async def arun(
        ...
        """run()의 async 버전 — LangGraph astream에서 호출."""
        tool_logs: list[dict] = []
        retry = 0
        current_query = query
        ...
        while True:
            ...
            start = time.time()
            try:
                raw = await search_cases.ainvoke({"query": current_query})
                case_chunks = raw.get("case_chunks", [])
            except Exception as e:
                logger.error("CaseAgent async retrieve 실패: %s", e)
                case_chunks = []
            ...
            if not case_chunks:
                if retry >= max_retries:
                    break
                current_query = await self._arewrite(current_query, item_text, risk_type, tool_logs)
                retry += 1
                continue

            # 2. Grade
            relevant = await self._agrade(case_chunks, item_text, risk_type, tool_logs)

            if relevant or retry >= max_retries:
                logger.info(
                    "CaseAgent async: %d건 검색 → %d건 관련 사례 확보 (retry=%d)",
                    len(case_chunks), len(relevant), retry,
                )
                return {"case_chunks": relevant, "tool_logs": tool_logs}
        ...
        logger.warning("CaseAgent async: 관련 사례 없음 (max_retries=%d 소진)", max_retries)
        return {"case_chunks": [], "tool_logs": tool_logs}
    ...
    async def _agrade(
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            raw_content = (chunk.get("content") or "")[:400]
            content = _strip_case_header(raw_content)
        ...
        try:
            ...
            logger.error("CaseAgent async grade 실패, 전체 포함: %s", e)
            relevant = chunks
    ...
    async def _arewrite(
        ...
        try:
            ...
            logger.error("CaseAgent async rewrite 실패: %s", e)
            new_query = old_query
    ...
    def _grade(
        ...
        for i, chunk in enumerate(chunks, 1):
            ...
            if meta.get("violation_type"):
                header_parts.append(f"위반유형: {meta['violation_type']}")
            header = " | ".join(header_parts) if header_parts else "정보없음"
            # content(=display_text)에서 [처리번호:...] 헤더 라인 제거하여 이중 노출 방지
            raw_content = (chunk.get("content") or "")[:400]
            content = _strip_case_header(raw_content)
            cases_text_parts.append(f"[사례 {i}] {header}\n내용: {content}")
